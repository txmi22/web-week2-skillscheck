...
let doubled = myNumbers.map(numDoubled)
function numDoubled(num){
	return num * 2;
}
...
let total = [3, 55, 788, 2, 1].reduce(function (accumlator, currentValue) {
...
let forTheLoveOfGeorge = notGeorge.map(function newName(George) {
	return George;
})
...
let totallyAwesome = people.reduce(function(awesomePeople){
	return awesomePeople += awesomeLevel
})
